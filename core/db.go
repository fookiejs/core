...
// kindSQL maps fookie kinds to PostgreSQL column types.
...
func (d *db) update(model *storedModel, id string, row map[string]any) (map[string]any, error) {
	if len(row) == 0 {
		return d.read(model, id)
	}
	var sets []string
	var args []any
	n := 1
	// Use model.fields order for deterministic SQL.
	for _, f := range model.fields {
		if f.Name == "id" {
			continue
		}
...
// encodeVal converts Go values to PostgreSQL-compatible representations.
// JSON fields are marshalled to a JSON string so pgx sends them as text.
...
// collectRows scans a pgx.Rows into []map[string]any using column names as keys.
