...
// Telemetry writes structured logs and metrics to two PostgreSQL tables:
//
//	fookie_logs    — one row per operation (request/response + error detail)
//	fookie_metrics — one row per operation (model, operation, latency, success)
//
// All writes are async via a buffered channel; the main request path is never blocked.
// If the buffer is full the entry is silently dropped (backpressure protection).
...
// ensureTables creates fookie_logs and fookie_metrics if they don't exist.
...
// record enqueues a telemetry entry.  Never blocks the caller.
...
// worker drains the channel and writes batches to PostgreSQL.
