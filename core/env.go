...
// loadEnv reads the env var.  Missing or empty vars are silently skipped
// (loaded=false) — callers check e.loaded before using e.value.
func (e *EnvVar[T]) loadEnv() error {
	...
	if !ok || raw == "" {
		return nil // optional — caller falls back to built-in default
	}
	if len(e.oneOf) > 0 {
		valid := false
...
// Value returns the loaded env value.  Panics if not loaded — use ValueOr for optional vars.
...
// ValueOr returns the loaded value, or fallback if the env var was not set.
