...
import { models, defaults } from "@fookiejs/core";
  ...
  for (const model of models) {
    ...
    for (const field of lodash.keys(schema)) {
      if ((schema[field] as any).relation) {
        ...
        resolvers[relatedModel.getName()]["all_" + model.getName()] =
          async function (parent: any, payload: any, context: any) {
        ...
        resolvers[relatedModel.getName()]["sum_" + model.getName()] =
          async function (parent: any, payload: any, context: any) {
            ...
            if (Array.isArray(items)) {
              sum = items.reduce(
                (total, item: any) =>
                  total + (Number((item as any)[payload.field]) || 0),
                0
              );
            }
        ...
        resolvers[relatedModel.getName()]["count_" + model.getName()] =
          async function (parent: any, payload: any, context: any) {
            const query: any = lodash.omit(payload.query, field);
            if (!query.filter) {
              query.filter = {};
  ...
  for (const typeName in typeDefs.type) {
    ...
    for (const field in typeDefs.type[typeName]) {
      if (typeDefs.type[typeName][field].all) {
        ...
        result += `  ${field}(query: ${model.getName()}_query): ${
