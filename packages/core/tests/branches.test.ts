...
import { MockDb, httpPost, httpRaw, LiveApps, serveApp } from "./mock-db.ts";
...
describe("branch coverage", () => {
  ...
  it("covers resume, external outbox branches, and nested persist", async () => {
    ...
    const parent = Model({
      ...
      flow: {
        ...
        async update(flow) {
          ...
          const updated = await flow.update(child, { id: { eq: created.id }, title: { eq: "u" } }, {
            title: "u2",
          });
    ...
    assert.equal(
      (await fookie.update(parent, { id: { eq: parentCreated.id } }, {})).signal,
      "done",
    );
    assert.equal(
    ...
    assert.equal(
      (await fookie.update(parent, { id: { eq: missingId } }, {})).signal,
      "done",
    );
  ...
  it("covers http edge cases and mutation signals", async () => {
    ...
    const failed = await httpPost(port, "/edge/update", {
      body: { loc: [2, 3] },
      filter: { email: { eq: "e@d.com" } },
    });
    assert.equal(failed.json.signal, "done");
  ...
  it("covers failed external resume and update coordinate body", async () => {
    ...
    await fookie.update(
      user,
      { email: { eq: "u@u.com" }, id: { eq: created.id } },
      { loc: [3, 4] },
    );
  });
  ...
  it("covers remaining http, outbox, and nested failure paths", async () => {
    ...
    const badUpdateFilter = await httpPost(port, "/left/update", {
      body: { email: "x@y.com" },
      filter: { email: { eq: 1 } },
    });
    ...
    const parent = Model({
      ...
      flow: {
        async create(flow) {
          return (await flow.update({ name: "Ghost" }, {}, {})).signal;
        },
        async list(flow) {
    ...
    if (created.signal === "done") {
      await fookie.update(user, { id: { eq: created.id }, email: { eq: "cache@t.com" } }, {
        email: "cache@t.com",
      });
      await fookie.update(user, { id: { eq: "00000000-0000-7000-8000-000000000000" } }, {});
    }
  });
  ...
  it("covers model ref binding, db cache miss, and filter edge branches", async () => {
    ...
    const refParent = Model({
      ...
      flow: {
        ...
        async update(flow) {
          ...
          const badFilter = await flow.update(
            refChild,
            { title: { eq: 1 } },
            { title: "u2" },
          );
          const badBody = await flow.update(
            refChild,
            { id: { eq: created.id }, title: { eq: "u" } },
            { title: "x", owner: "not-a-uuid" },
          );
    ...
    if (parentCreated.signal === "done") {
      await fookie.update(refParent, { id: { eq: parentCreated.id } }, {});
    ...
    await fookie2.update(
      cacheUser,
      { id: { eq: created.id }, email: { eq: "cache@x.com" } },
      { email: "cache@y.com" },
    );
    ...
    assert.equal(
      (
        await fookie.update(
          cacheUser,
          { id: { eq: created.id }, email: { eq: "cache@y.com" } },
          { email: "not-an-email" },
        )
      ).signal,
      "failed",
