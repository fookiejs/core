...
describe("dispatcher and failure reporting", () => {
  ...
  it("arms its timer on run and clears it on stop without holding the process open", async () => {
    ...
    const timer = (fookie as unknown as { dispatcherTimer: { hasRef(): boolean } | undefined })
      .dispatcherTimer;
    assert.ok(timer !== undefined, "run armed the dispatcher");
    assert.equal(timer.hasRef(), false, "the timer is unref'd");

    await fookie.stop();
    assert.equal(
      (fookie as unknown as { dispatcherTimer: unknown }).dispatcherTimer,
      undefined,
      "stop cleared it",
    );
  });
...
describe("create result and http surface", () => {
  ...
  it("settles an external failure through the app API, not HTTP", async () => {
    const db = new MockDb();
    const seen: Seen[] = [];
    ...
    const port = await serveApp(fookie);
    ...
    const blocked = await httpPost(port, "/external/failure", {
      externalId,
      reason: "template missing",
      failure: "permanent",
    });
...
describe("flow.pg", () => {
  it("runs the caller's query inside the flow's own transaction", async () => {
    ...
    const beginAt = db.queries.findIndex((sql) => sql.startsWith("BEGIN"));
