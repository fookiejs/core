...
import { MockDb, httpPost, LiveApps, serveApp } from "./mock-db.ts";
...
describe("order flow integration", () => {
  ...
  it("updates and deletes orders like example flows", async () => {
    ...
    const updateSignal = await fookie.update(
      order,
      { id: { eq: row.id }, status: { eq: "confirmed" } },
      { status: "draft" },
    );
  ...
  it("serves order http api mirroring example models", async () => {
    ...
    const updateRes = await httpPost(port, "/order/update", {
      body: { status: "draft" },
    ...
    assert.equal(extRes.status, 404);
  });
