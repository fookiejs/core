...
describe("final coverage", () => {
  ...
  it("covers model ref binding, persist failures, and mutation validation", async () => {
    ...
    if (c.signal === "done") {
      db.mode = "fail-upsert";
      await f3.update(coord, { id: { eq: c.id }, email: { eq: "c@c.com" } }, { loc: [3, 4] });
      db.mode = "ok";
      await f3.delete(coord, { id: c.id, filter: { email: { eq: "c@c.com" } } });
    }

    assert.equal(
      (await fookie.update(parent, {}, { email: "not-email" })).signal,
      "failed",
    );
    assert.equal(
      (await fookie.update(parent, { email: { eq: 1 } }, {})).signal,
      "failed",
    );
    assert.equal(
  ...
  it("covers http create failed, path edge cases, and filter parsing", async () => {
    ...
    const invalidBody = await httpPost(port, "/httpfinal/update", {
  ...
  it("covers remaining runtime and http branches", async () => {
    ...
    const wrapper = Model({
      ...
      flow: {
        ...
        async update(flow) {
          ...
          db.mode = "fail-upsert";
          const updated = await flow.update(
            child,
    ...
    await fookie.update(wrapper, { id: { eq: "00000000-0000-7000-8000-000000000099" } }, {});
    ...
    await serveApp(f3);
    const slash = await httpPost(port + 2, "/httpleft/x/update", {
      filter: {},
      body: { email: "a@b.com" },
    });
    ...
    db.mode = "fail-select";
    const miss = await fookie.update(
      parent,
      { id: { eq: "00000000-0000-7000-8000-000000000000" } },
      { email: "n@e.com" },
    );
  ...
  it("covers http request error and json coordinate upsert", async () => {
    ...
    await fookie.update(
      jsonUser,
      { id: { eq: created.id }, email: { eq: "j@c.com" } },
      { data: "{}" },
    );
