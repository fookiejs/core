...
describe("postgres row locking", () => {
  ...
  it("does not lose a concurrent update to a different field", async () => {
    ...
    const [alphaDone, betaDone] = await Promise.all([
      writer.update(counter, { id: { eq: created.id } }, { alpha: 1 }),
      second.update(counter, { id: { eq: created.id } }, { beta: 1 }),
    ]);
  ...
  it("breaks a deadlock instead of hanging on it", async () => {
    ...
    const settled = await Promise.all([
      left.update(crossing, { id: { eq: first.id } }, { alpha: 1 }),
      right.update(crossing, { id: { eq: second.id } }, { alpha: 1 }),
    ]);
