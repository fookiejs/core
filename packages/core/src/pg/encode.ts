...
export type QueryResultRow = Record<string, HostValue>;
...
export function pgRowCells(row: QueryResultRow): Record<string, PgParam> {
  ...
  for (const [key, raw] of Object.entries(row)) {
    ...
    if (pgScalarSchema.safeParse(raw).success === true) {
      ...
      present = Number.isFinite(raw.getTime()) === true;
    } else if (Buffer.isBuffer(raw) === true) {
      present = true;
    } else if (Array.isArray(raw) === true) {
      present = true;
    } else if (isPlainRecord(raw) === true) {
      present = true;
    }
    if (present === false) {
      continue;
