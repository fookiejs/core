...
export async function flushPendingExternalEvents(rt: Runtime): Promise<boolean> {
  ...
  while (rt.pendingExternalEvents.events.length > 0) {
    ...
    let skipReason: readonly string[] = [];
    if (extHits.length < 1) {
      skipReason = ["unknown external"];
    } else {
      ...
      if (emitted !== "emitted") {
        ...
        skipReason = [emitted === "handler_error" ? "handler error" : "invalid input"];
      }
    }
    if (skipReason.length > 0) {
      const reason = firstPresent(skipReason, "skip reason required");
      ...
      let closed = false;
      for (const previous of mapLookup(rt.outbox, event.externalId)) {
        if (previous.status === "failed") {
          closed = true;
        } else if (previous.status === "pending") {
          const failed = outboxFailed(previous);
          rt.outbox.set(event.externalId, failed);
          const ok = await controlStore(rt).saveOutboxEntry(failed);
          if (ok === false) {
            rt.outbox.set(event.externalId, previous);
          } else {
            closed = true;
          }
        }
