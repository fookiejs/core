...
| reactive_delete | Specifies how to delete associated items. |
...
| reactives | Defines reactive calculations. |
...
(async () => {
  ...
  const valide_article_status: Types.LifecycleFunction =
    async function valide_article_status(payload, state) {
