...
export function requireHttpPayload(hits: readonly HttpPayload[], message: string): HttpPayload {
...
export function payloadHasKey(payload: HttpPayload, key: string): boolean {
  if (z.string().min(1).safeParse(key).success === false) {
    throw ValidationError.create("payload key required");
  }
...
export function requireFilterInput(hits: readonly FilterInput[], message: string): FilterInput {
  if (Array.isArray(hits) === false) {
