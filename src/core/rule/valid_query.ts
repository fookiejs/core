...
export default async function (payload, state) {
    const accepted_keywords = ctx.local.get("setting", "accepted_query_field_keys")
    let filter_keys = ctx.lodash.keys(payload.query.filter)
    let model_keys = ctx.lodash.keys(state.model.schema)
    if (ctx.lodash.has(payload.query, "filter") && !ctx.lodash.isObject(payload.query.filter)) return false
    if (ctx.lodash.has(payload.query, "limit") && !ctx.lodash.isNumber(payload.query.limit)) return false
    if (ctx.lodash.has(payload.query, "offset") && !ctx.lodash.isNumber(payload.query.offset)) return false
    if (ctx.lodash.difference(filter_keys, [...model_keys, state.database.pk]).length > 0) return false

    for (let key of ctx.lodash.keys(payload.query.filter)) {
        let field = payload.query.filter[key]
        if (
            ctx.lodash.isObject(field) &&
            ctx.lodash.difference(ctx.lodash.keys(field), accepted_keywords.value.keys).length != 0
        ) {
            return false
        }
